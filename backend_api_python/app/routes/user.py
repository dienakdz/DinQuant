...
def get_roles():
    ...
    for role in service.ROLES:
        roles.append({"id": role, "name": role.capitalize(), "permissions": service.get_user_permissions(role)})
...
def test_notification_settings():
    ...
    try:
        ...
        if failed:
            ...
            logger.warning(
                "notification_settings test: user_id=%s failed_channels=%s errors=%s", user_id, failed, err_detail
            )
