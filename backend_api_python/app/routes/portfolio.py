...
def update_position(position_id):
    ...
    try:
        ...
        with get_db_connection() as db:
            cur = db.cursor()
            cur.execute(f"UPDATE qd_manual_positions SET {', '.join(updates)} WHERE id = ? AND user_id = ?", params)
            db.commit()
            cur.close()
...
def add_monitor():
    ...
    try:
        ...
        notification_config_json = json.dumps(
            notification_config if isinstance(notification_config, dict) else {}, ensure_ascii=False
        )
...
def update_monitor(monitor_id):
    ...
    try:
        ...
        if "notification_config" in data:
            ...
            params.append(
                json.dumps(notification_config if isinstance(notification_config, dict) else {}, ensure_ascii=False)
            )
        ...
        with get_db_connection() as db:
            cur = db.cursor()
            cur.execute(f"UPDATE qd_position_monitors SET {', '.join(updates)} WHERE id = ? AND user_id = ?", params)
            db.commit()
            cur.close()
...
def delete_monitor(monitor_id):
    ...
    try:
        ...
        with get_db_connection() as db:
            cur = db.cursor()
            cur.execute("DELETE FROM qd_position_monitors WHERE id = ? AND user_id = ?", (monitor_id, user_id))
            db.commit()
            cur.close()
...
def run_monitor_now(monitor_id):
    ...
    try:
        ...
        if async_mode:
            ...
            thread = threading.Thread(target=run_in_background, args=(monitor_id, language, user_id), daemon=True)
            thread.start()
...
def update_alert(alert_id):
    ...
    try:
        ...
        with get_db_connection() as db:
            cur = db.cursor()
            cur.execute(f"UPDATE qd_position_alerts SET {', '.join(updates)} WHERE id = ? AND user_id = ?", params)
            db.commit()
            cur.close()
...
def delete_alert(alert_id):
    ...
    try:
        ...
        with get_db_connection() as db:
            cur = db.cursor()
            cur.execute("DELETE FROM qd_position_alerts WHERE id = ? AND user_id = ?", (alert_id, user_id))
            db.commit()
            cur.close()
...
def get_groups():
    ...
    try:
        ...
        return jsonify({"code": 1, "msg": "success", "data": {"groups": groups, "ungrouped_count": ungrouped_count}})
    except Exception as e:
