...
- 简要说明原因，不要冗长"""
            ...
            analyzed_markets.sort(key=lambda x: x.get("ai_analysis", {}).get("opportunity_score", 0), reverse=True)
    ...
    def _fallback_analysis(self, markets: List[Dict], max_opportunities: int) -> List[Dict]:
        ...
        opportunities.sort(key=lambda x: x.get("ai_analysis", {}).get("opportunity_score", 0), reverse=True)
                    ...
                    try:
                        ...
                        """,
                            (
                                ...
                                float(
                                    ai_analysis.get("predicted_probability", market.get("current_probability", 50.0))
                                ),
