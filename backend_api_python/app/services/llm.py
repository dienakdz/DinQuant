...
class LLMService:
    ...
    def _call_google_gemini(
        self, messages: list, model: str, temperature: float, api_key: str, base_url: str, timeout: int
    ) -> str:
            ...
            try:
                if p == LLMProvider.GOOGLE:
                    ...
                    return self._call_openai_compatible(
                        messages, current_model, temperature, api_key, base_url, timeout, use_json_mode=use_json_mode
                    )
                ...
                if status_code in (402, 403) and try_alternative_providers and current_model == models_to_try[-1]:
                    ...
                    return self._try_alternative_providers(
                        messages, original_model, temperature, use_json_mode, excluded_provider=p
                    )
    ...
    def _try_alternative_providers(
        self, messages: list, model: str, temperature: float, use_json_mode: bool, excluded_provider: LLMProvider = None
    ) -> str:
        """
    ...
    def call_openrouter_api(
        self, messages: list, model: str = None, temperature: float = 0.7, use_fallback: bool = True
    ) -> str:
    ...
    def safe_call_llm(
        ...
        try:
            ...
            try:
                ...
                logger.debug(f"Failed to extract JSON substring: {e}")
