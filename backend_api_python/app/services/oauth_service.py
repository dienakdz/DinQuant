...
class OAuthService:
            ...
            with get_db_connection() as db:
                ...
                if email:
                    ...
                    if existing_user:
                        ...
                        cur.execute("UPDATE qd_users SET last_login_at = NOW() WHERE id = ?", (existing_user["id"],))
                        db.commit()
                        cur.close()
        ...
        try:
            with get_db_connection() as db:
                ...
                if not user or not user["password_hash"]:
                    ...
                    cur.execute("SELECT COUNT(*) as count FROM qd_oauth_links WHERE user_id = ?", (user_id,))
                    count = cur.fetchone()["count"]
                    if count <= 1:
                        cur.close()
                        return False, "Cannot unlink the only authentication method"

                cur.execute("DELETE FROM qd_oauth_links WHERE user_id = ? AND provider = ?", (user_id, provider))
                db.commit()
                cur.close()
    ...
    def cleanup_expired_states(self, max_age_minutes: int = 10):
        ...
        expired = [k for k, v in self._states.items() if v.get("created_at", datetime.now()) < cutoff]
        for k in expired:
            del self._states[k]
