...
class BacktestService:
    ...
    def run_multi_timeframe(
        ...
        try:
            ...
            metrics = self._calculate_metrics(
                equity_curve, trades, initial_capital, timeframe, start_date, end_date, total_commission
            )
    ...
    def _simulate_trading_mtf(
        ...
        for i, (timestamp, row) in enumerate(df_exec.iterrows()):
            ...
            while signal_queue_idx < len(signal_queue):
                ...
                if timestamp >= sig_effective_time:
                    ...
                    if can_execute:
                        ...
                        pending_signal_time = sig_effective_time  # noqa
                        signal_queue_idx += 1
            ...
            equity_curve.append(
                {"time": timestamp.strftime("%Y-%m-%d %H:%M"), "value": round(max(0, current_equity), 2)}
            )
    ...
    def _fetch_kline_data(
        self, market: str, symbol: str, timeframe: str, start_date: datetime, end_date: datetime
    ) -> pd.DataFrame:
        ...
        try:
            ...
            if data_start > start_date:
                logger.warning(
                    f"Requested start date {start_date} is before available data start {data_start}. "
                    f"Using available start date instead."
                )
            if data_end < end_date:
                logger.warning(
                    f"Requested end date {end_date} is after available data end {data_end}. "
            ...
            if data_end < end_date:
                ...
                if len(df) > requested_candles:
                    ...
                    logger.warning(
                        f"Available data ({len(df)} candles) is less than requested ({requested_candles} candles). "
            ...
            if df_filtered.empty:
                logger.error(
                    f"After filtering date range ({effective_start} to {effective_end}), no data remains. "
                    f"Available data range: {data_start} to {data_end}, requested: {start_date} to {end_date}"
                )
                return pd.DataFrame()

            logger.info(
                f"After filtering: {len(df_filtered)} candles remain for backtest (effective range: {effective_start} to {effective_end})"
            )
            return df_filtered
...
"""
    ...
    def _execute_script_strategy(
        self, code: str, df: pd.DataFrame, runtime: Optional[Dict[str, Any]] = None
    ) -> Dict[str, pd.Series]:
        runtime = runtime or {}
        ...
        try:
            ...
            safe_builtins = {
                ...
                for k in dir(builtins)
                if not k.startswith("_")
                and k not in ["eval", "exec", "compile", "open", "input", "help", "exit", "quit"]
            }
            safe_builtins["__import__"] = safe_import
    ...
    def _simulate_trading_new_format(
        ...
        for i, (timestamp, row) in enumerate(df.iterrows()):
            ...
            equity_curve.append({"time": timestamp.strftime("%Y-%m-%d %H:%M"), "value": round(total_value, 2)})
    ...
    def _format_result(self, metrics: Dict, equity_curve: List, trades: List) -> Dict[str, Any]:
        ...
        return {**cleaned_metrics, "equityCurve": cleaned_curve, "trades": cleaned_trades}
